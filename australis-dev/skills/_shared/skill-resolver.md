...
3. Fallback: read `.australis/skill-registry.md` from the project root.
4. No registry found → proceed without project skills. The registry is rebuilt automatically by the `explore` phase on the next `/nuevo`.
...
- /absolute/path/to/project/.claude/skills/typescript/SKILL.md
```
...
- The registry persists in Engram (when available) and `.australis/skill-registry.md`.
...
- **Australis Orchestrator**: resolves paths for all SDD and non-SDD delegations.
